...
# ── 1. Зависимости ──
echo -e "${G}[1/6]${X} Системные зависимости..."
...
# ── 2. Claude Code CLI ──
echo -e "${G}[2/6]${X} Claude Code CLI..."
...
# ── 4. Директории ──
echo -e "${G}[4/6]${X} Директории..."
mkdir -p ~/.claude/cicada_v/{logs,peers}
mkdir -p ~/cicada_v-vault

# ── 5. Mesh-ключ (не трогаем существующий) ──
echo -e "${G}[5/6]${X} Ключ и команда..."

mkdir -p ~/.claude/cicada_v
if [ ! -f ~/.claude/cicada_v/mesh.key ] || [ ! -s ~/.claude/cicada_v/mesh.key ]; then
    echo "cicada" > ~/.claude/cicada_v/mesh.key
    chmod 600 ~/.claude/cicada_v/mesh.key
    echo -e "  ◬ Установлен ключ по умолчанию: cicada"
else
    echo -e "  ◬ Ключ не тронут (уже есть в mesh.key)"
fi

# ── 6/6. Команда cicada_v ──
echo -e "${G}[6/6]${X} Команда..."
...
#!/bin/bash
# CICADA_V bootstrap — скачивает REPL и запускает
RUS_DIR="$HOME/.claude/cicada_v"
mkdir -p "$RUS_DIR/tmp"

# Не трогаем существующий ключ окружения
if [ -z "$ANTHROPIC_AUTH_TOKEN" ] && [ -f "$RUS_DIR/mesh.key" ] && [ -s "$RUS_DIR/mesh.key" ]; then
